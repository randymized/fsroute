...
function requestHandler(fsRoute,context,next)
{
  ...
  if (list) {
    ...
    var descend= function(err) {
      ...
      if (i == list.length) {
        if (context.on_no_determinate && !list.determinate) {
      ...
      var handler= list[i++]
      if (handler.indeterminate) {
        return fsRoute.indeterminate_handler_caller(handler,context,descend)
      }
      else {
        return fsRoute.determinate_handler_caller(handler,context,descend)
      }
    }
    return descend()
  }
